...
var unsupportedOpenMetricsMetricTypes = map[string]struct{}{
	"INFO":     {},
	"STATESET": {},
}

func openMetricsTimestamp(token string) (*timestamppb.Timestamp, error) {
	ts, _, err := big.ParseFloat(token, 10, 256, big.ToNearestEven)
	if err != nil {
		return nil, err
	}
	nanos := new(big.Float).Mul(ts, big.NewFloat(1e9))
	nanosInt, acc := nanos.Int(nil)
	if acc != big.Exact {
		return nil, fmt.Errorf("cannot represent %q as nanoseconds", token)
	}
	if !nanosInt.IsInt64() {
		return nil, fmt.Errorf("timestamp %q out of range", token)
	}
	totalNanos := nanosInt.Int64()
	secs := totalNanos / 1e9
	nsec := totalNanos % 1e9
	if nsec < 0 {
		secs--
	...
	return timestamppb.New(time.Unix(secs, nsec)), nil
}

func (p *OpenMetricsParser) setCreatedTimestamp(dst **timestamppb.Timestamp) bool {
	*dst, p.err = openMetricsTimestamp(p.currentToken.String())
	if p.err == nil {
		return true
	}
...
func (p *OpenMetricsParser) readingValue() stateFn {
	...
	case dto.MetricType_COUNTER:
		if p.currentIsExemplar {
			...
			} else if !p.setCreatedTimestamp(&p.currentMetric.Counter.CreatedTimestamp) {
				return nil
			}
		}
	...
	case dto.MetricType_SUMMARY:
		...
		case p.currentIsMetricCreated && !p.setCreatedTimestamp(&p.currentMetric.Summary.CreatedTimestamp):
			return nil
...
func (p *OpenMetricsParser) startTimestamp() stateFn {
	...
	if p.currentIsExemplar {
		p.currentExemplar.Timestamp, p.err = openMetricsTimestamp(p.currentToken.String())
		if p.err != nil {
			p.parseError(fmt.Sprintf("expected float as timestamp, got %q", p.currentToken.String()))
			return nil
		}
	} else {
...
func (p *OpenMetricsParser) getOrCreateMetric(metrics map[uint64]*dto.Metric) *dto.Metric {
	signature := model.LabelsToSignature(p.currentLabels)
	if metric := metrics[signature]; metric != nil {
		return metric
	}
	metrics[signature] = p.currentMetric
	p.currentMF.Metric = append(p.currentMF.Metric, p.currentMetric)
	return p.currentMetric
}
...
func (p *OpenMetricsParser) skipBlankTabUntilNewline() {
	if p.currentByte != '\n' {
		for {
			if p.currentByte, p.err = p.buf.ReadByte(); p.err != nil || p.currentByte == '\n' || !isBlankOrTab(p.currentByte) {
				return
			}
		}
	}
