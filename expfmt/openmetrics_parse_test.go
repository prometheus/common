					...
					Metric: []*dto.Metric{
						{
							Counter: &dto.Counter{
								...
								CreatedTimestamp: timestamppb.New(time.Unix(123456, 700000000)),
							},
						},
					},
				},
...
`,
		...
		// 8: Counter with exemplar
		{
			in: `# TYPE foo counter
# HELP foo help
foo_total{b="c"} 0 123456 # {a="b"} 0.5 123456.7
# EOF
`,
			out: []*dto.MetricFamily{
				{
					...
					Metric: []*dto.Metric{
						{
							...
							Counter: &dto.Counter{
								...
								Exemplar: &dto.Exemplar{
									...
									Timestamp: timestamppb.New(time.Unix(123456, 700000000)),
								},
							},
							TimestampMs: proto.Int64(123456),
						},
					...
					Metric: []*dto.Metric{
						{
							Summary: &dto.Summary{
								...
								CreatedTimestamp: timestamppb.New(time.Unix(123456, 0)),
							},
						},
					},
				},
								...
								Bucket: []*dto.Bucket{
									{
										...
										Exemplar: &dto.Exemplar{
											...
											Value:     proto.Float64(0.5),
											Timestamp: timestamppb.New(time.Unix(123456, 0)),
										},
									},
									{
										UpperBound:      proto.Float64(0.1),
					...
					Metric: []*dto.Metric{
						{
							...
							Counter: &dto.Counter{
								...
								CreatedTimestamp: timestamppb.New(time.Unix(123456789, 0)),
							},
						},
						{
							Label: []*dto.LabelPair{
					...
					Metric: []*dto.Metric{
						{
							Histogram: &dto.Histogram{
								...
								CreatedTimestamp: timestamppb.New(time.Unix(123456789, 123000000)),
								Bucket: []*dto.Bucket{
									{
...
func TestOpenMetricsSkipBlankTabUntilNewline(t *testing.T) {
	var p OpenMetricsParser
	p.buf = bufio.NewReader(strings.NewReader("  #"))

	p.skipBlankTabUntilNewline()
	if p.err != nil {
		t.Fatalf("unexpected error: %v", p.err)
	}
	if p.currentByte != '#' {
		t.Fatalf("expected helper to skip all blanks and stop at '#', got %q", p.currentByte)
	}
}
